...
fun main() {
    val initConfig = datagramInitConfig {
        localAddress("http://127.0.0.1:9124")
        ...
        initAdapter {
            encoderFactories(
                ...
                object : MessageToMessageEncoder<ByteBuf>() {
                    override fun encode(ctx: ChannelHandlerContext?, msg: ByteBuf?, out: MutableList<Any>?) {
                        if (ctx == null || msg == null || out == null) return
                        out += DatagramPacket(msg.copy(), InetSocketAddress("255.255.255.255", 9123))
                    }
                },
    ...
    chatClient.start {
        chatClient.writeValue(
            ...
            channelContextMatcher = ChannelContextMatcher { channelContext ->
                channelContext.channelRemoteAddress == initConfig.localSocketAddress
            },
